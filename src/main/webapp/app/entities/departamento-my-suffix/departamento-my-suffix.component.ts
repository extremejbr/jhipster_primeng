...
@Component({
    selector: 'jhi-departamento-my-suffix',
    templateUrl: './departamento-my-suffix.component.html'
})
export class DepartamentoMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.departamentoService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DepartamentoMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/departamento-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/departamento-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: DepartamentoMySuffix) {
        return item.id;
    }
